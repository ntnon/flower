...
export class VectorMatrices {
    ...
    updatePath() {
        ...
        let acc = 0
        this.vectors.map((v, index) => acc += v.vectors.length)
        console.log(acc)
    }
...
export class VectorMatrix {
    ...
    createCircularPattern(
        ...
        for (let i = 0; i < repetitions; i++) {
            ...
            const group: Vector[] = this.vectors.map(({ x, y }) => this.applyRotationAndTranslation(x, y, angle, centerX, centerY));
    ...
    private applyRotationAndTranslation(x: number, y: number, angle: number, centerX: number, centerY: number): Vector {
